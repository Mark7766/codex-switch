...
Codex Switch 从「代理工具」正式变成**纯配置工具**：不再需要它在后台运行，配置写好后各工具
直连供应商。

### 主要变化

- **不用再开着应用了**。本地转发服务已彻底移除，Codex / Claude 直连供应商，应用关掉也照常可用。
- **智谱 GLM 也能直连**，不再经过本地转发。
- **DeepSeek 模型精简为两个**：**Flash**（日常首选，现在自己就能看图）和 **V4 Pro**（更强）。
  提醒：V4 Pro 将在北京时间 2026-09-14 12:00 之后由 Flash 接管。
- **Agnes 已下线**，请改用 DeepSeek 或智谱 GLM。
- **界面更聚焦**：「设置」成为默认首屏，主面板只显示四个工具的接入状态；插件页、日志页、
  帮助页、搜索按钮已移除。
...
> 在设置里把 Claude Desktop 从智谱 GLM 切换到 DeepSeek（或反过来），点「保存并应用」，配置文件可能没有被修改——Claude Desktop 里看到的还是旧供应商的模型。

- **切换供应商后配置文件强制写入**。不再依赖「检测到 Claude Desktop 已安装」才写配置——只要你主动点了保存，不管检测结果如何，配置一定会写入。没装 Claude Desktop 就先写好了等你装，装了就直接生效
- **模型映射不再残留**。切换供应商时，之前存的旧供应商模型映射自动清空，点保存后模型自动用新供应商的默认值（例如切到 DeepSeek 自动用 deepseek-v4-pro / deepseek-v4-flash）。想自定义映射也可以打开「管理模型映射」自行调整
- **少了 Key 会明确提示**。之前只有 Agnes 和 GLM 缺 Key 才提示，DeepSeek 缺 Key 时点保存静默失败——现在三个供应商缺 Key 都会明确告诉你「请先在供应商设置中配置 Key」
- **写入失败会报错了**。之前配置写入失败在后台悄悄吞掉，你看到的始终是「保存成功」——现在写入失败会直接在界面上显示具体错误原因

## [1.14.2] - 2026-06-21

### 新增：长对话自动续命 🎉

> 聊到一半突然报「对话过长」中断？现在代理会自动帮你处理——把太早的历史精简掉，对话继续，你什么都感觉不到。

- **上下文超限自动截断重试**。DeepSeek 报"聊太长了"时，代理不再直接丢错误给你看，而是自动裁剪最早的旧消息、保留最近 30 条关键对话，然后静默重试。绝大多数情况下对话无缝继续，你完全无感知
- **工具调用结果也不会丢**。裁剪时自动保留相关的工具配对，不会出现工具结果孤零零没上下文的情况
- **日志里能看见**。如果自动截断发生了，Codex Switch 日志页会显示一条「自动截断 N 条旧消息后重试」，方便排查

### 修复

- **供应商切换后模型选啥就是啥**。切换供应商时选择的默认模型不再被覆盖——选 glm-4.7 就是 glm-4.7，不会偷偷变成 glm-5.2
- **macOS 升级不再懵圈**。升级新版本后如果提示「已损坏」，点安装按钮时会弹出修复指引，下载文件夹里还会自动生成一个双击即修复的脚本
- **修复了长对话中的三个隐蔽问题**（同版本持续迭代）：
  - 截断重试失败时 Codex 不再卡在「等待中」转圈——现在正确发送 `response.failed` 事件让 Codex 正常结束
  - 截断后不再出现孤立工具调用导致 DeepSeek 协议错误——工具调用和结果的配对清理现在覆盖双向
  - 修复了对话历史在被截断重试后仍可能因消息重复而超限的问题——消息累积逻辑不再重复拼接历史记录
...
> Agnes AI 是一个免费 API 供应商，无需付费，支持 256K 上下文、工具调用。现在 Codex Switch 完整支持——设置里选 Agnes，填 Key，点保存，全家桶一起切。

- **多供应商支持**。设置页新增四张独立接入卡片：「供应商设置」统一管理 Key、「Codex 接入」「Claude Desktop 接入」「Claude Code CLI 接入」——每张卡片独立选择 DeepSeek 或 Agnes，互不干扰
- **Agnes + Codex**。供应商选 Agnes 后，代理自动翻译 OpenAI Responses → Chat Completions，Codex 无感知，直接对话
- **Agnes + Claude Desktop / Code CLI**。Claude 全家桶选 Agnes 后自动走 Codex Switch 代理，Anthropic Messages → Chat Completions 翻译在代理层完成，Claude 端无需改模型名
- **模型映射弹窗**。Claude 卡片新增「管理模型映射」，可单独配置每个 Claude 模型对应哪个实际模型
- **供应商秒切不重启**。切供应商只改代理内存映射，Codex 和代理都不需要重启

### 新增

- **🔍 智能搜索**。右上角新增搜索按钮，输入问题 AI 秒回答案，支持拖拽调整窗口大小
- **FAQ 新增 Agnes**。帮助文档新增「如何接入免费的 Agnes AI 模型」，从注册到配置三步走
...
- **长对话不再报 compact 错误**。代理直接返回"无需压缩"，不再 404/502
- **上下文超限给中文提示**。超出上限时弹出人话建议
- **保存配置时自动写入窗口优化设置**。告诉 Codex "你有 1M 窗口"
