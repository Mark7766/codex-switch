...
### M5 依赖评估结果（2026-06-13）

**已更新（安全）**:

- `electron-updater`: 6.8.3 → 6.8.9（patch）
- `prettier`: 3.8.3 → 3.8.4（dev，patch）

**高风险大版本（不建议盲目升级）**:

| 包                     | 当前   | 最新   | 风险                                              |
| ---------------------- | ------ | ------ | ------------------------------------------------- |
| `electron`             | 30.5.1 | 42.4.0 | 跨 12 个大版本，API 大量变更                      |
| `electron-store`       | 8.2.0  | 11.0.2 | 已 deprecated，需迁移到 `@electron-toolkit/store` |
| `react`                | 18.3.1 | 19.2.7 | 破坏性变更，服务端组件 API                        |
| `typescript`           | 5.9.3  | 6.0.3  | 主版本更新，需回归测试                            |
| `zustand`              | 4.5.7  | 5.0.14 | 公开 API 变化显著                                 |
| `vite`                 | 5.4.21 | 8.0.16 | CJS API deprecated                                |
| `vitest`               | 1.6.1  | 4.1.8  | 配置文件格式变化                                  |
| `eslint`               | 8.57.1 | 10.5.0 | 扁平化配置，所有插件需更新                        |
| `@typescript-eslint/*` | 7.18.0 | 8.61.0 | 配合 eslint v9+                                   |
| `tailwindcss`          | 3.4.19 | 4.3.1  | CSS 配置格式完全重写                              |

**建议**：当前版本组合已验证稳定，大版本升级应作为独立项目评估。
